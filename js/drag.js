...
define(['$', '_vue'], function($, Vue) {
    ...
    function overlap(drag, origin) {
        let offset1 = offset(drag)

        let $_origins = $(origin)
        let dom = []

        $.each($_origins, function(i, el){
            $.each($_origins, function(j, parent){
                let flag = true
                if (el !== parent) {
                    let closest = $(el).closest(parent)
                    if (closest.length > 0) {
                        dom = [].concat(el, dom)
                        flag = false
                    }
                ...
                if (flag) {
                    let status = true
                    for(let index = 0, size = dom.length; index < size; index++) {
                        if(dom[index] === el) {
                            status = false
                            break
                        }
